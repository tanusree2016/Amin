...
class Profile extends Component {
  ...
  handleSubmit(e) {
    ...
    base_url.post('admin-account/', profileApi, {
      headers: {
        'x-access-token': localStorage.getItem('token'),
      }
    })
      .then(res => {
        console.log("Response --- "+JSON.stringify(res.data.admins.companyName))
        if (res.data.value == 1) {
          localStorage.setItem('companyName', res.data.admins.companyName);
          localStorage.setItem('contact', res.data.admins.contact);
          localStorage.setItem('profilepic', res.data.admins.profile);
          this.resetField();
  ...
  render() {
    ...
    console.log("PICTURE --- "+localStorage.getItem('profilepic'));
    if (localStorage.getItem('profilepic') != 'img') {
      console.log('ur here');
      ...
      console.log('ur here if else --- else');
      $imagePreview = (<img src={imagePreviewUrl} style={{ width: "190px", height: "190px", borderRadius: "100px" }} />);
    } else {
