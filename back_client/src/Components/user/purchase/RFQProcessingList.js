...
class RFQProcessingList extends Component {
    ...
    searchItemDisplay() {
        ...
        if (c == 0) {
            for (let i = 0; i < this.state.getAllRequisition.length; i++) {
                if (this.state.getAllRequisition[i].status === "rfq")
                    this.state.getAllRequisition[i].status = "Send for Quotation"
    ...
    fetchAllRFQProcessingRequisition() {
        let id = localStorage.getItem('id');
        this.setState({ loading: true }, () => {
            fetch(envirionment.BASE_URL + 'rfq', {
                method: 'get',
                headers: {
                ...
                .then(res => {
                    console.log(res)
                    this.setState({
                        getAllRequisition: res.RFQRequisitionList,
                        loading: false,
                    });
                    console.log("Requisition Details --- " + JSON.stringify(res.RFQRequisitionList))
    ...
    componentDidMount() {
        ...
        this.fetchAllRFQProcessingRequisition();
    }
                        ...
                        <Table>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell style={tableHeadStyleQuantity}>Required Quantity</TableCell>
...
export default connect(mapStateToProps)(RFQProcessingList)
