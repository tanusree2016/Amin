        ...
        bcrypt.genSalt(10, (err, salt) => {
            ...
            bcrypt.hash(req.body.password, salt, (err, hash) => {
                ...
                const Provid = new Provider({
                    ...
                    final_category: req.body.childsubcategory,
                    experience: req.body.experience,
                    salary_hr: req.body.salary,
                    is_verified: 'verified'

                });

                Provid.save().then(providers => {
                    if (!providers) {
                        res.json({ value: 0, message: "not saved" });
                    }
                    else {
                        res.json({ value: 1, message: "success", data: providers });
                    }
                    // console.log("===="+JSON.stringify(providers));
                })

                //console.log("==================================================")
                // console.log("provide========="+JSON.stringify(Provid))
            });
...
router.post('/delete-provider', function (req, res) {
    console.log('--' + JSON.stringify(req.body))
    const values = {
        is_deleted: 1
    }
    ...
    Provider.update(values, selector).then(update => {
        if (!update) {
            res.json({ value: 0, message: 'not deleted' })
        }
        else {
...
router.post('/provider-update', function (req, res) {
